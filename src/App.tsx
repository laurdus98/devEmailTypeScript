...
import {
  IGettingSuccessAction,
  IGettingFailureAction
} from "./devEmail/redux/actions/actions";
...
import { MailAPI } from "./devEmail/apis/fakeAPI/MailAPI";
import { MessaggiAPI } from "./devEmail/apis/fakeAPI/MessaggiAPI";
import { CategoriaAPI } from "./devEmail/apis/fakeAPI/CategoriaAPI";
...
const App: FC<IProps> = ({
  ...
  const [payloadMail, setPayloadMail] = useState<Mail[]>([]);

  const [payloadMessaggi, setPayloadMessaggi] = useState<Messaggio[]>([]);

  const [payloadCategoria, setPayloadCategoria] = useState<Categoria[]>([]);
      ...
      fetchingMessage("/Messaggi")
        .then((resp: IGettingSuccessAction<Messaggio>) => {
          const {
            ...
            status,
            type
          }: IGettingSuccessAction<Messaggio> = resp;
          if (status === "OK" && type === "Success") {
            setPayloadMessaggi(payload);
          } else {
            setPayloadMessaggi(MessaggiAPI.messaggi);
          }
        })
        .catch((error: IGettingFailureAction) => console.log(error));
      fetchingCategoria("/Categoria")
        .then((resp: IGettingSuccessAction<Categoria>) => {
          const {
            ...
            status,
            type
          }: IGettingSuccessAction<Categoria> = resp;
          if (status === "OK" && type === "Success") {
            setPayloadCategoria(payload);
          } else {
            setPayloadCategoria(CategoriaAPI.categorie);
          }
        })
        .catch((error: IGettingFailureAction) => console.log(error));
      setFetch(true);
    }
  }, [isFetch, setFetch, fetchingMail, fetchingMessage, fetchingCategoria]);
